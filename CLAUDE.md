...
- Get session in Server Components via `auth.api.getSession({ headers: await headers() })` from `src/lib/auth.ts`.
- Get session in Client Components via `useSession()` from `src/lib/auth-client.ts`.
- Never access the session cookie directly — use the auth helpers above.
